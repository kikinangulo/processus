<?php
    ...
    namespace Processus\Lib\Db
    {
        ...
        class MySQL implements \Processus\Interfaces\InterfaceDatabase
        {
             ...
             * @param array $values
             * @param bool $insertIngore
             * @return \Zend\Db\Statement\Pdo
             * @throws \Processus\Exceptions\MySqlException
             */
            public function insert($tableName = NULL, $values = array(), $insertIngore = FALSE)
            {
                if (!empty($tableName) && !empty($values)) {
                    ...
                    // use INSERT IGNORE for unique indexes
                    if($insertIngore === TRUE)
                    {
                ...
                throw new \Processus\Exceptions\MySqlException("Insert data are wrong! Required -> Table name && to inserted values");
            }
